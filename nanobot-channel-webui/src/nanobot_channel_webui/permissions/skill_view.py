    ...
    def render(self, skill_name: str, policy: PolicyContext | None) -> str | None:
        ...
        lines.extend(
            [
                ...
                "- For create/update/delete capabilities, first prepare one JSON plan, then present a business preview and wait for explicit user confirmation. Use the same plan content with `hr_business` only after the user confirms.",
                "- For actions that require `input`, prefer passing the JSON plan directly as the `input` string to `hr_business`; the tool stores it in the current chat runtime automatically. Do not search for runtime-inputs directories.",
