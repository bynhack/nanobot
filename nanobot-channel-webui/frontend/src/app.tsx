...
const DETAIL_PANEL_WIDTH_KEY = 'nanobot_channel_webui_detail_panel_width';
...
function buildTextAppendMessage(text: string): AppendMessage {
  return {
    role: 'user',
...
function MessageAttachmentChip({
  ...
  return (
    <button type="button" className="message-attachment-chip" onClick={() => openMedia(item)}>
      <span className="icon" dangerouslySetInnerHTML={{ __html: fileTypeIcon(mime) }} />
...
function Composer({
  ...
  const applySelectedSkill = useCallback(
    (skillName: string) => {
      ...
      // Clear the textarea synchronously so the DOM value is empty before the
      // chip re-mounts the input; otherwise a quick Backspace would see the
      // stale /query text still in the DOM.
      applyComposerInputValue(nextDraft);
      setDraft(nextDraft);
      setInputRevision((current) => current + 1);
      onSelectSkill(skillName);
    },
    [applyComposerInputValue, onSelectSkill, readComposerInputValue],
  );
  ...
  const handleDraftKeyDown = useCallback(
    (event: KeyboardEvent<HTMLTextAreaElement>) => {
      if (event.key === 'Backspace' && selectedSkillName && !event.currentTarget.value) {
...
export function App() {
  ...
  const pendingAskUserPrompt = useMemo(() => {
    ...
    return findPendingAskUserPrompt(state.messagesByChat[chatId] ?? []);
  ...
  const handleNewMessage = useCallback(
    async (message: AppendMessage, options: { applySelectedSkill?: boolean } = {}) => {
      const content = extractTextInput(message);
      const applySelectedSkill = options.applySelectedSkill ?? true;
      const effectiveContent =
        applySelectedSkill && selectedSkillName && !content.trimStart().startsWith('$')
  ...
  const sessionGroups = useMemo(
    () => groupSessionsForSidebar(state.sessions, nowMs),
    [nowMs, state.sessions],
  );
            ...
            <div className="sessions-list">
              {sessionGroups.length ? (
                sessionGroups.map((group) => (
                  <section key={group.key} className="session-group" aria-label={group.label}>
                    ...
                    <div className="session-group-list">
                      {group.sessions.map((session) => (
                        ...
                        >
                          <p className="session-title">{session.preview}</p>
                          {session.read_only ? <span className="session-pill">只读</span> : null}
                          {!session.read_only ? (
                            <button
                              type="button"
                ...
                <div className="sidebar-empty-state">
                  <div className="session-group-label">最近</div>
                  <p>还没有历史对话，点击上方新对话开始使用。</p>
                </div>
              )}
            </div>
          </div>
                      ...
                      {pendingAskUserPrompt ? (
                        <div className="ask-user-block">
                          <AskUserPromptCard
                            ...
                            onAnswer={(answer) => {
                              void handleNewMessage(buildTextAppendMessage(answer), {
                                applySelectedSkill: false,
                              });
                            }}
                          />
                        </div>
