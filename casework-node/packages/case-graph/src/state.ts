...
export function normalizeLayout(
  ...
  for (const [rawId, rawPoint] of Object.entries(record(raw.nodePositions))) {
    ...
    if (id && point && (!allowed.size || allowed.has(id)))
      positions[id] = point;
  }
  ...
  for (const [rawId, rawMeta] of Object.entries(record(raw.positionMeta))) {
    ...
    if (
      !["initial", "generated", "manual", "group", "restored"].includes(source)
    )
      continue;
  ...
  for (const [rawGroupId, rawState] of Object.entries(
    record(raw.groupLayout),
  )) {
    ...
    for (const [rawId, rawPoint] of Object.entries(
      record(state.memberPositionsBeforeCollapse),
    )) {
      const id = text(rawId);
      const point = normalizePoint(rawPoint);
      if (id && point && (!allowed.size || allowed.has(id)))
        memberPositionsBeforeCollapse[id] = point;
    }
    groupLayout[groupId] = {
...
export function normalizeGraphBody(value: unknown): JsonRecord {
  ...
  for (const [key, item] of Object.entries(record(raw.tradeFacts)))
    if (text(key) && isRecord(item)) tradeFacts[text(key)] = record(item);
  return {
    nodes,
    ...
    investigationGroups: list(raw.investigationGroups)
      .filter(isRecord)
      .map(record),
    ...
    filters: {
      minAmount: Object.hasOwn(record(raw.filters), "minAmount")
        ? record(raw.filters).minAmount
        : null,
      maxAmount: Object.hasOwn(record(raw.filters), "maxAmount")
        ? record(raw.filters).maxAmount
        : null,
      startTime: text(record(raw.filters).startTime),
      endTime: text(record(raw.filters).endTime),
    },
    drillNums: positiveInt(raw.drillNums, 10),
    drillType:
      raw.drillType == null || raw.drillType === "" ? 1 : raw.drillType,
    ...
    graphData: Object.hasOwn(raw, "graphData") ? raw.graphData : null,
  };
}
...
export function normalizeGraphDocument(payload: JsonRecord): JsonRecord {
  return {
    ...
    graphId: text(payload.graphId),
    graphName: text(payload.graphName),
    revision: Math.trunc(Number(payload.revision || 0)),
    updatedAt: text(payload.updatedAt) || nowIso(),
    lastStepId: text(payload.lastStepId),
    metadata: record(payload.metadata),
    graph: normalizeGraphBody(payload.graph),
  };
}

export function legacyGraphToDocument(
  caseId: string,
  graphId: string,
  graph: JsonRecord,
  graphName = "",
