...
import type { HistoryMessage } from "@casework/contracts";
...
export class SessionStore {
  ...
  async get(chatId: string): Promise<StoredSession | null> {
    ...
    try {
      return JSON.parse(
        await readFile(this.path(chatId), "utf8"),
      ) as StoredSession;
    } catch (error) {
  ...
  async list(
    ...
    Array<Omit<StoredSession, "messages"> & { message_count: number }>
  > {
  ...
  async append(
    ...
    const session =
      (await this.get(chatId)) ?? (await this.createWithId(chatId, ownerId));
    session.messages.push(message);
    session.last_ts = new Date().toISOString();
    const text = "content" in message ? message.content.trim() : "";
    if (text)
      session.preview = text.slice(0, 60) + (text.length > 60 ? "…" : "");
    await this.save(session);
    return session;
  }
