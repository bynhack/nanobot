...
export class MediaService {
  ...
  constructor(
    ...
    this.secret = createHash("sha256")
      .update(secret || "casework-node-development-secret")
      .digest();
  }
  ...
  issue(path: string): string {
    const payload: MediaTokenPayload = {
      path: resolve(path),
      exp: Math.floor(Date.now() / 1000) + Math.max(1, this.ttlSeconds),
    };
  ...
  async resolve(token: string): Promise<string | null> {
    try {
      ...
      if (!payload.path || payload.exp < Math.floor(Date.now() / 1000))
        return null;
