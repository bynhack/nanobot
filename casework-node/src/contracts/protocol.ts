...
export type HistoryMessage =
  ...
  | { id?: string; type: 'tools'; tools: Array<{ name: string; args: Record<string, unknown>; result: string; status: 'ok' | 'error' }> }
...
export function parseClientCommand(raw: string): ClientCommand | null {
  ...
  if (value.type === 'message.cancel' || value.type === 'session.new') return { type: value.type };
  ...
  return attachments.length ? { type: 'message.send', content, attachments } : { type: 'message.send', content };
}
