...
export function evidenceLevel(
  ...
  if (operationType.startsWith("investigation_group_")) {
    return groupOperation === "collapse" ||
      groupOperation === "expand" ||
      ["investigation_group_collapse", "investigation_group_expand"].includes(
        operationType,
      )
      ? "automatic"
      : "optional";
...
export function normalizeEvidence(
  ...
  if (reasonCode === "other" && !note)
    throw new Error("选择其他依据时，请补充具体说明");
  return {
    level,
    ...(reasonCode
      ? {
          reasonCode: reasonCode.slice(0, 80),
          reasonLabel: reasonLabel.slice(0, 120),
        }
      : {}),
...
export function evidenceFromBusinessFields(
  reasonLabel: string,
