...
import type { BootstrapConfig, ConnectionState, MediaItem, SessionSummary } from './types';
...
const RECENT_SESSION_WINDOW_MS = 3 * 24 * 60 * 60 * 1000;
...
function sessionTimestamp(session: SessionSummary): number {
  const raw = session.last_ts ?? session.created_at;
  if (!raw) {
    return 0;
  }
...
export function groupSessionsForSidebar(
  ...
  for (const session of sessions) {
    if (now - sessionTimestamp(session) <= RECENT_SESSION_WINDOW_MS) {
      recent.push(session);
    } else {
  ...
  if (recent.length) {
    groups.push({ key: 'recent', label: '最近', sessions: recent });
  }
  if (earlier.length) {
    groups.push({ key: 'earlier', label: '更早', sessions: earlier });
  }
  return groups;
}
