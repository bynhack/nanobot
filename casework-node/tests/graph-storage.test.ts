...
import { CaseGraphStorage } from '../src/case-graph/case-graph-storage.js';
import { GraphRepository } from '../src/case-graph/graph-repository.js';

test('case graph snapshots preserve the public state shape', async () => {
  const root = await mkdtemp(join(tmpdir(), 'casework-node-'));
  const storage = new CaseGraphStorage(join(root, 'graphs'), join(root, 'contexts'));
...
test('relation step storage externalizes trade facts and preserves existing positions', async () => {
  ...
  const result = await repository.appendStep({ caseId: '37', graphId: 'g1', graphName: '图一', operation: { type: 'seed_one_hop' }, graph, delta: { addedNodes: graph.nodes, addedEdges: graph.edges }, summary: {} });
  ...
  const stored = JSON.parse(await readFile(join(root, 'graphs', '37', 'g1', 'graph.json'), 'utf8'));
  assert.equal(stored.graph.tradeFacts, undefined);
  assert.equal(stored.graph.factStore.tradeFactsPath, 'facts/trades.jsonl');
  assert.match(await readFile(join(root, 'graphs', '37', 'g1', 'facts', 'trades.jsonl'), 'utf8'), /"tradeId":"t1"/);
});
