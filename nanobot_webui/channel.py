...
from nanobot_webui.auth import WebUIAccessControl
from nanobot_webui.connections import ConnectionRegistry
from nanobot_webui.management import WebUIManagementService
...
class WebUIConfig(Base):
    ...
    host: str = "127.0.0.1"
    port: int = 8080
    allow_from: list[str] = Field(default_factory=lambda: ["*"])  # Deprecated; browser auth is token/origin based.
...
class WebUIHook(AgentHook):
    ...
    async def after_iteration(self, ctx: AgentHookContext) -> None:
        ...
        await self._registry.emit_to_chat(chat_id, turn_completed_event(chat_id, content=content))
